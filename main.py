...
print(max_value)
h = (max_value + 0.0001 * max_value - min_value) / m  # шаг, с которым идут интервалы
...
tinv = 1.2816  # посчитано в MATLAB функцией tinv(0.9, n-1), 0.9 = (1+q)/2, где q=0.8 CHANGE
...
for i in range(1, 11):
    ...
    dispersion_interval.append(help.dispersion_interval(numOfPointsInOneUnderArray, dispersion[i], left_chi2inv, right_chi2inv))
...
for i in range(m):
    ...
    chi2_stat += (numOfPoints * ___Pk - _nk[index]) / (numOfPoints * ___Pk)
    index += 1
...
for i in range(m):
    if i == 0:
        ___Pk = stats.norm.cdf(steps[index], loc=c_for_normal_mmp, scale=s_for_normal_mmp)/stats.norm.cdf(min_value, loc=c_for_normal_mmp, scale=s_for_normal_mmp)
    else:
        ___Pk = stats.norm.cdf(steps[index], loc=c_for_normal_mmp, scale=s_for_normal_mmp) - stats.norm.cdf(steps[index - 1], loc=c_for_normal_mmp, scale=s_for_normal_mmp)
